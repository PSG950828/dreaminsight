 ...
 * - 한국어 조사 매칭(koHit) / 확장 사전 팩 / 질문 즉답 / GI 정규화 / Markdown 내보내기
 */
...
// 사전 병합
const SYMBOLS: Record<string, SymbolMeaning> = { ...BASE_SYMBOLS };
...
function extractMatches(tokens: string[], raw: string) {
  ...
  const emotions = [
    ...Object.entries(EMOTION_LEXICON)
  ...
  const colors = [
    ...Object.entries(COLOR_CUES)
      .filter(([k]) => tokens.includes(k))
      .map(([k, v]) => ({ key: k, cue: v })),
    ...Object.entries(COLOR_KO)
  ...
  const actions = [
    ...Object.entries(ACTION_PATTERNS)
      .filter(([k]) => tokens.includes(k))
      .map(([, v]) => v),
    ...Object.entries(ACTION_KO)
      .filter(([k]) => text.includes(k))
      .map(([, v]) => v),
  ];
...
function analyzeDream(raw: string): Analysis {
  ...
  if (colors.some((c) => c.key === "black" || c.key === "검정"))
    patterns.push("무의식/두려움 활성화");
  if (colors.some((c) => c.key === "blue" || c.key === "파랑"))
    patterns.push("진정/사고 정렬 필요");
  ...
  const R: CompositeRule[] = [
    // 1~10: 확장/통제/감정
    {
      keys: [T(["비행기", "airplane", "비행"]), T(["추락", "떨어짐", "falling"])],
      pattern: "확장 시도 중 통제 상실 두려움",
      advice: "최악/최상 시나리오와 완충 버퍼 1개 설정.",
    },
    {
      keys: [T(["물", "바다", "ocean", "water"]), T(["쓰나미", "해일", "tsunami", "파도"])],
      pattern: "감정의 폭발/범람",
      advice: "감정 강도(0~10) 기록 후 해소행동 1개.",
    },
    {
      keys: [T(["집", "house", "방"]), T(["불", "화재", "fire"])],
      pattern: "자아 경계와 안정성 위협",
      advice: "생활 영역 1곳 정리 + 갈등 1건 정리.",
    },
    {
      keys: [T(["시험", "exam"]), T(["지각", "늦음", "late"])],
      pattern: "완벽주의 + 준비부족 불안",
      advice: "‘충분히 괜찮음’ 기준으로 샘플 먼저 제출.",
    },
    {
      keys: [T(["길", "도로", "road"]), T(["길잃음", "lost", "헤맴"])],
      pattern: "방향성 혼란/의사결정 회피",
      advice: "선택지 3개·1년 후 모습 1문장씩 시뮬.",
    },
    {
      keys: [T(["뱀", "snake"]), T(["물림", "물리", "bite"])],
      pattern: "유혹·위험과 실제 갈등 촉발",
      advice: "불편한 대화 주제 1개 메모→전달 준비.",
    },
    {
      keys: [T(["아기", "아이", "baby"]), T(["울음", "cry"])],
      pattern: "새 프로젝트와 돌봄/유지 부담",
      advice: "초안→시작→유지 3단계 분해.",
    },
    {
      keys: [T(["죽음", "death"]), T(["장례", "funeral"])],
      pattern: "종결/전환의 수용",
      advice: "끝낼 일 1건 종결 선언 후 에너지 재배분.",
    },
    {
      keys: [T(["고양이", "cat"]), T(["검정", "black"])],
      pattern: "직관/독립 욕구 vs 불안",
      advice: "직관적 선택 1개를 합리화 없이 실행.",
    },
    {
      keys: [T(["전화", "phone", "핸드폰"]), T(["못받", "부재", "미응답"])],
      pattern: "소통 욕구와 기회 상실 두려움",
      advice: "미뤄둔 메시지 1건 전송.",
    },
    // 11~20: 관계/자아
    {
      keys: [T(["거울", "mirror"]), T(["수치", "창피"])],
      pattern: "자기이미지 취약/자존감 흔들림",
      advice: "자기긍정 1문장 + 잘한 일 3가지 기록.",
    },
    {
      keys: [T(["어머니", "엄마", "mother"]), T(["병원", "hospital"])],
      pattern: "감정영양/돌봄 자원 고갈 경보",
      advice: "보살핌 주고받기 균형 체크 3항목.",
    },
    {
      keys: [T(["아버지", "아빠", "father"]), T(["분노", "화"])],
      pattern: "권위/기준과의 충돌",
      advice: "규칙 1개만 선택·적용.",
    },
    {
      keys: [T(["연인", "배우자", "partner"]), T(["문", "door", "닫힘"])],
      pattern: "관계 경계 재설정 필요",
      advice: "요청/감사 1문장 메시지 전송.",
    },
    {
      keys: [T(["전애인", "옛연인", "ex"]), T(["돈", "지갑", "money"])],
      pattern: "가치/교환 비교에서 온 자기의심",
      advice: "그 관계에서 배운 가치 3가지 목록화.",
    },
    {
      keys: [T(["낯선", "stranger"]), T(["추격", "쫓김", "chase"])],
      pattern: "그림자 측면 회피",
      advice: "미룬 과제 1건 25분 착수 후 감정 기록.",
    },
    {
      keys: [T(["개", "dog"]), T(["문", "경계"])],
      pattern: "보호본능과 경계 균형",
      advice: "시간/연락/업무 경계선 1문장 선언.",
    },
    {
      keys: [T(["호랑이", "tiger"]), T(["집", "house"])],
      pattern: "자아/가정 영역 리더십 호출",
      advice: "회의/대화에서 입장 1번 명확히 표명.",
    },
    {
      keys: [T(["거미", "spider"]), T(["망", "웹", "엮"])],
      pattern: "복잡성/의존성 높은 작업 과부하",
      advice: "의존그래프 작성→첫 노드 수행.",
    },
    {
      keys: [T(["피", "blood"]), T(["계단", "stairs"])],
      pattern: "에너지 소모 속 점진 성장 압박",
      advice: "오늘 ‘한 칸’만 진행 + 10분 휴식.",
    },
    // 21~30: 일/성과/시간
    {
      keys: [T(["시계", "time", "시한"]), T(["시험", "exam"])],
      pattern: "시간압박 하의 평가 스트레스",
      advice: "25분 타이머로 샘플 먼저.",
    },
    {
      keys: [T(["컴퓨터", "PC"]), T(["홍수", "overflow", "메일"])],
      pattern: "정보 과부하/인박스 폭주",
      advice: "인박스 10분 클리어.",
    },
    {
      keys: [T(["지갑", "wallet"]), T(["염려", "불안"])],
      pattern: "재정 불안",
      advice: "계좌 확인 + 구독 1건 중지.",
    },
    {
      keys: [T(["열쇠", "key"]), T(["잠금", "닫힘"])],
      pattern: "접근권한/실마리 차단",
      advice: "핵심 질문 1개 작성→요청 발송.",
    },
    {
      keys: [T(["반지", "ring"]), T(["금색", "gold"])],
      pattern: "약속·가치 재서약",
      advice: "이번 주 약속 1건 서면 확정.",
    },
    {
      keys: [T(["돈", "money"]), T(["시장", "교환"])],
      pattern: "가치/교환 집착 또는 결핍",
      advice: "불필요 지출 1건 건너뛰기.",
    },
    {
      keys: [T(["창문", "window"]), T(["파랑", "blue"])],
      pattern: "차분한 정보 유입/환기 필요",
      advice: "신뢰 가능한 채널 1개 열기.",
    },
    {
      keys: [T(["신발", "shoes"]), T(["길", "road"])],
      pattern: "역할/여정 정렬 필요",
      advice: "역할 맞춤 준비 1건 먼저 수행.",
    },
    {
      keys: [T(["버스", "bus"]), T(["지각", "늦음"])],
      pattern: "집단 페이스 의존으로 지연",
      advice: "나만의 20분 집중 블럭 확보.",
    },
    {
      keys: [T(["기차", "train"]), T(["표", "티켓"])],
      pattern: "장거리 계획 승인/확정 단계",
      advice: "장기 목표 1개 일정 고정.",
    },
    // 31~40: 자연/재난/전환
    {
      keys: [T(["산", "mountain"]), T(["눈", "snow"])],
      pattern: "장기 도전 + 고립감/체력 저하",
      advice: "체크포인트 3개·보상 1개 설정.",
    },
    {
      keys: [T(["숲", "forest"]), T(["길", "나무"])],
      pattern: "무의식 탐색 중 방향 재설정",
      advice: "10분 걷기 + 떠오르는 단어 5개 기록.",
    },
    {
      keys: [T(["사막", "desert"]), T(["갈증", "목마름"])],
      pattern: "자원 고갈/의미 추구",
      advice: "지원 요청 1건 발송.",
    },
    {
      keys: [T(["강", "river"]), T(["다리", "bridge"])],
      pattern: "전환의 문턱/연결 욕구",
      advice: "미뤄둔 연락 1건으로 연결 복원.",
    },
    {
      keys: [T(["배", "ship"]), T(["폭풍", "storm"])],
      pattern: "감정 항해 중 외부 변수 급증",
      advice: "우선순위 3개로 축소 + 안전항로 지정.",
    },
    {
      keys: [T(["엘리베이터", "elevator"]), T(["추락", "정지"])],
      pattern: "빠른 단계 이동 실패 공포",
      advice: "가속 대신 안전장치 1개 설치.",
    },
    {
      keys: [T(["계단", "stairs"]), T(["올라감", "up"])],
      pattern: "점진 성장의 재확인",
      advice: "오늘 한 칸 완료 체크.",
    },
    {
      keys: [T(["계단", "stairs"]), T(["내려감", "down"])],
      pattern: "과거/무의식으로 하강",
      advice: "옛 감정 트리거 1개 글로 적기.",
    },
    {
      keys: [T(["지진", "earthquake"]), T(["집", "건물"])],
      pattern: "기반 구조 재설계 필요",
      advice: "핵심 시스템(돈/시간) 1개 리팩터링.",
    },
    {
      keys: [T(["홍수", "flood"]), T(["전화", "통신"])],
      pattern: "감정 범람으로 소통 장애",
      advice: "연락창구 1개만 열어 과부하 차단.",
    },
  ];

  const lower = raw.toLowerCase();
  const hasWord = (variants: string[]) =>
    variants.some((v) => lower.includes(v) || koHit(lower, v));
  for (const rule of R) {
    const ok = rule.keys.every((group) => hasWord(group));
    if (ok) {
      patterns.unshift(rule.pattern);
    }
  }
  ...
  if (emotions.includes("불안"))
    adviceBase.add("호흡 4-7-8을 3회 반복하고, 불안 근원을 문장으로 라벨링하세요.");
  if (patterns.includes("미해결 과제 회피"))
    adviceBase.add("미뤄둔 과제 1개를 25분 타이머로 착수하세요 (포모도로).");
  ...
  const summary = foundSymbols.length
    ? `주요 상징 ${foundSymbols.map((s) => s.label).join(", ")} 포착. ${
        patterns.join(", ") || "패턴 파악은 추가 기록 권장"
      }.`
  ...
  if (isQuestion) {
    const topSym = foundSymbols[0]?.label
      ? `주요 상징: ${foundSymbols.map((s) => s.label).join(", ")}. `
      : "";
    const topPat = patterns[0] ? `핵심 패턴: ${patterns.join(", ")}. ` : "";
    const next = adviceBase.size
      ? `바로 할 일: ${Array.from(adviceBase)[0]}`
      : "바로 할 일: 감정 라벨링 3문장.";
...
function saveJournals(list: Journal[]) {
  try {
    ...
    alert("저장 공간이 가득 찼습니다. 일부 기록을 삭제한 뒤 다시 시도하세요.");
  }
}
...
function toMarkdown(j: Journal) {
  ...
  const col =
    a.colors.map((c) => `${c.key}(${c.cue})`).join(", ") || "(색 단서 없음)";
...
export default function DreamInsightApp() {
  ...
  const invalidMsg = useMemo(() => {
    const v = text.trim();
    if (v.length === 0)
      return "꿈 내용을 입력하세요. 상징/감정/색/행동 중 2가지를 포함하면 좋습니다.";
    ...
    <div className="min-h-screen bg-gradient-to-b from-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-950 text-zinc-900 dark:text-zinc-50">
      <div className="max-w-md mx-auto px-4 py-6 sm:py-10">
                ...
                <Card className="rounded-xl border-dashed">
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {draftAnalysis.symbols.map((s, i) => (
                        <Pill key={i}>{s.label}</Pill>
                      ))}
                      {draftAnalysis.emotions.map((e, i) => (
                        <Pill key={"e" + i}>{e}</Pill>
                      ))}
                      {draftAnalysis.colors.map((c, i) => (
                        <Pill key={"c" + i}>{c.key}</Pill>
                      ))}
                    </div>
                  </CardContent>
                </Card>
        ...
        <TabsContent value="history" className="mt-3 space-y-3">
          {journals.length === 0 && (
            <p className="text-sm opacity-70">
              아직 저장된 기록이 없습니다. 꿈을 입력하고 해석을 생성해 보세요.
            </p>
          )}
            ...
            <motion.div key={j.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="rounded-2xl border-zinc-200/60 dark:border-zinc-800/60">
                ...
                <CardContent className="space-y-4 text-sm">
                  ...
                  <Section title="상징 해석">
                    <div className="space-y-2">
                      {j.analysis.symbols.length === 0 && (
                        <p className="opacity-70">(상징 없음)</p>
                      )}
                      {j.analysis.symbols.map((s, i) => (
                        <div
                          key={i}
                          className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200/60 dark:border-zinc-800/60"
                        >
                          <div className="font-medium mb-1">{s.label}</div>
                          <div className="text-[13px] leading-6 opacity-90">{s.meaning}</div>
                          <div className="text-[12px] mt-2">
                            <span className="opacity-70">태그:</span> {s.tags.join(", ")}
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <Section title="감정/색/행동 단서">
                    <div className="flex flex-wrap gap-2">
                      {j.analysis.emotions.map((e, i) => (
                        <Pill key={"em" + i}>{e}</Pill>
                      ))}
                      {j.analysis.colors.map((c, i) => (
                        <Pill key={"co" + i}>
                          {c.key}:{c.cue}
                        </Pill>
                      ))}
                      {j.analysis.actions.map((a, i) => (
                        <Pill key={"ac" + i}>{a}</Pill>
                      ))}
                      {j.analysis.emotions.length +
                        j.analysis.colors.length +
                        j.analysis.actions.length === 0 && (
                  ...
                  <Section title="저널 프롬프트">
                    <ul className="list-disc list-inside space-y-1">
                      {j.analysis.journalingPrompts.map((q, i) => (
                        <li key={i}>{q}</li>
                      ))}
                    </ul>
                  </Section>
                  ...
                  <div className="text-[11px] opacity-60">
                    GI Score(내부 계산): {j.analysis.giScore}
                  </div>
                </CardContent>
              </Card>
            </motion.div>
        ...
        <div className="mt-6 text-center text-[11px] opacity-60">
          © {new Date().getFullYear()} DreamInsight
        </div>
      </div>
    </div>
  );
